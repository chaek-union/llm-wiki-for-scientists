...
이 책을 읽는 사람들은 학생, 연구원, 교수들로 예상된다. 우선 파트 1은 모든 독자들이 읽어야 한다. 각 챕터를 순서대로 읽는다. 기본적인 배경설명을 시작으로, 에이전트 설치, LLM-Wiki 구축, 활용 방법을 정리했다.
...
파트3은 연구책임자들을 위해 작성했다. 실제로 내가 평소에 쓰던 방식 - 수많은 잡일과 행정적인 부담으로부터 나를 해방시켜준 일종의 해방일지와 같다. 생존 가이드라인이다. 이렇게 일하다 죽을순 없으니.. 에이전트와 LLM-Wiki가 어떻게 연구책임자의 일을 덜어주는지를 정리했다.

...
셋째는 연구책임자로 연구실을 운영하는 방식이다. 사람이 늘고 과제가 늘면 지금 무엇이 어디까지 왔는지를 묻는 일 자체가 일이 된다. 답이 한 자리에서 나오지 않고 메일과 달력과 공유 문서와 메신저에 흩어져 있기 때문이다. 그래서 각 자리가 무엇을 소유하는지 정하고 위키는 그것을 가리키게 두었다. 논문을 읽는 위키 옆으로 원고를 쓰는 폴더와 분석을 하는 폴더가 갈라져 나온 것도 같은 이유다. 각각에 폴더 배치와 작업 규칙을 정해 두고 무엇을 왜 바꿨는지를 작업 로그로 남기면, 세 종류의 작업이 서로 다른 자리를 갖고 각 자리가 자기 규칙과 자기 기록을 소유한다. 사람에 관한 기록도 같은 원칙으로 쓴다. 날짜가 붙은 사실만 쌓고 점수나 등급은 만들지 않으며, 오래된 관찰은 현재 상태로 읽히지 않도록 표시가 내려간다. 매일 아침 판단할 것을 모아 주는 자리를 하나 두었더니 흩어진 것을 찾아다니는 시간이 줄었고, 그랜트와 행정 서류처럼 형식이 정해진 일에서는 자동화가 어디서 멈춰야 하는지를 정하는 일이 더 중요했다.

세 가지에 공통된 것이 하나 있다. 위키가 읽는 일과 말하는 일을 대신해 준 것이 아니라 그 두 가지를 할 자리를 만들어 주었다는 점이다. 강의에서 자주 듣는 걱정이 여기에 걸린다. AI로 논문을 읽는 것은 클릭 몇 번이고 그렇게 해서는 지식이 남지 않는다는 말인데, 맞는 말이면서 동의하지 않는다. 조건이 하나 붙는다. 처리 과정에서 나오는 내용을 이해되지 않더라도 눈으로 한 번은 읽어야 한다. 길지 않다. A4 반 장 정도다. 그 뒤에 어떤 질문이 생겨 위키에 물어보거나 발표를 준비하려고 그 문서를 다시 열게 되면 한쪽의 지식은 반드시 남는다. 무엇을 어디에 두고 무엇을 확인할지 정해 두지 않으면 결과물이 늘어나는 속도만 빨라지고 그중 무엇이 검증된 것인지는 알 수 없게 되므로, 이 책은 읽어야 하는 자리와 확인해야 하는 자리를 절차 안에 못 박아 두는 쪽으로 썼다.

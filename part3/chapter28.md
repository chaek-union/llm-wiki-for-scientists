...
반복된 누락은 정보가 모자라서 생긴 것이 아니었다. 답해야 할 메일과 오늘 마감인 것과 학생이 기다리고 있는 회신은 모두 어딘가에 적혀 있었고, 빠지는 것은 그날 아침에 어느 도구를 먼저 열었는지에 따라 달라졌다. 메일함부터 열면 일정이 늦게 보이고 일정부터 보면 사흘 된 메일이 하루 더 밀린다. 이 순서를 사람이 매일 다시 정하고 있었다는 것이 문제였고, 그래서 자동화가 대신하기로 한 일은 정보를 모으는 것이 아니라 순서를 고정하는 것이 되었다. 지금 보고가 시간이 정해진 것에서 시작해 사람이 기다리는 것, 내가 시작해야 하는 것, 시스템 상태로 내려가는 순서를 쓰는 이유가 그것이다.

정보를 더 모으는 방향으로 가지 않기로 한 근거는 이미 쌓여 있던 것에서 나왔다. 예약 작업이 만드는 결과 카드는 매일 늘어나는데 실행 상태 기록을 열어 보면 검토 대기 상태가 압도적으로 많았다. 읽히지 않는 산출물은 없는 것과 같고, 이 경우에는 그보다 나쁘다. 자동화가 실패했을 때 그것을 알아차리는 경로가 두 가지뿐인데 하나가 사람이 결과 카드를 여는 것이어서, 카드가 쌓이기만 하면 탐지 경로 하나를 스스로 닫는 셈이 된다. 실제로 조사하던 날 상류 작업이 자료를 받아 검증까지 마쳐 놓고 인계 파일만 만들지 않은 구멍이 벌어져 있었고, 그것을 잡아낸 것은 알림이 아니라 하류 보고서에 적힌 한 줄이었다. 설계된 탐지가 작동했지만 사람이 읽어야만 드러나는 구조였다는 뜻이다. 그래서 보고의 항목 수에 상한을 두었다. 스무 개를 다 올릴 수 있으면 우선순위 규칙은 시험받지 않고, 여덟 개만 올려야 할 때 무엇을 빼는지에서 그 규칙이 실제로 작동하는지가 드러난다.

...
외부 마감 하나는 그 자체로 실행할 수 없는 항목이다. 마감에서 거꾸로 세어 과학적 검토, 예산 확정, 행정 검토, 서명, 업로드, 최종 확인의 자리를 잡아야 비로소 일이 된다. 각 자리에는 그 일을 끝냈다고 말할 수 있는 조건을 붙인다. 예산 확정은 항목별 근거가 붙고 집행 규정에 어긋나지 않는지 확인된 상태이고, 최종 확인은 올라간 파일이 우리가 의도한 그 파일인지 다시 열어 본 상태다. 완료 조건을 적지 않으면 모든 항목이 마감 당일에 동시에 걸린다. 일정표에 과제 이름을 통째로 넣는 방식은 잘 작동하지 않는다. 큰 이름이 적힌 세 시간 블록은 무엇을 하면 끝나는지 알 수 없어서 다른 일에 잠식된다. 두 번째 Aim의 논리 수정, 예산 항목과 인건비 대조, 협력 기관 확약서 검토처럼 끝났는지 아닌지가 판정되는 이름을 붙인다. 일정이 바뀌면 세 곳이 함께 바뀌어야 한다. Timeline의 milestone, action register의 기한, 그리고 실제 일정표다. 셋 중 하나만 고치는 일이 반복되면 어느 것이 현재 계획인지 알 수 없게 되고, 마감이 가까워질수록 이 불일치가 비싸진다. 세 곳을 대조해 서로 어긋나는 항목, 담당이 없는 항목, 완료 조건이 비어 있는 항목, 이미 지난 기한을 찾아내는 일은 규칙이 분명하고 사람이 성실하게 하기 어려워서 에이전트에게 맡길 만하다. 다만 무엇을 미루고 무엇을 살릴지는 사람이 정한다.
...
그랜트를 운영하면서 반복되는 오해가 다섯 가지 있다. 첫째, 제안서를 잘 쓰면 관리가 된다고 보는 것이다. 잘 쓴 제안서는 선정 확률을 높이지만 선정 이후의 운영을 대신하지 않는다. 제안서에 적힌 약속은 이후 몇 해 동안 반복해서 확인되는 기준이 되므로, 그 약속이 무엇에 근거했는지가 함께 남아야 관리가 가능해진다. 둘째, 지난번 제출본에 있던 요구사항을 현재 규정으로 쓰는 것이다. 서식, 분량, 필수 서류, 자격 요건은 회차마다 바뀌고, 바뀐 것을 알려 주는 사람은 없다. 셋째, 정리 기록만 보고 제안서의 주장을 확정하는 것이다. 요약 단계에서 조건 하나가 빠진 문장은 읽기에 자연스럽지만 원문 앞에서는 과장이 된다. 제안서의 강한 주장과 인용은 원문으로 돌아가 다시 확인한다. 넷째, 확인하지 못한 항목을 이상 없음으로 적거나 아직 확정되지 않은 값을 채워 두고 나중에 고치겠다고 생각하는 것이다. 채워진 칸은 며칠만 지나도 확정값처럼 보이고, 여러 사람이 그 값을 근거로 다음 결정을 내린다. 확정되지 않은 예산, 참여 조건, 성과 수치는 비워 두고 미확인 라벨과 함께 누구에게 무엇을 요청한 상태인지를 적는다. 다섯째, 에이전트에게 맡길 수 있는 범위를 넓게 잡는 것이다. 에이전트는 문서 사이의 모순, 담당이 없는 항목, 지난 기한, 빠진 서류를 찾는 데 강하고 그 일에 쓰는 것이 이 장의 취지다. 제출 버튼을 누르는 일, 예산 항목을 정하는 일, 참여 인력을 바꾸는 일, 과제 폴더를 다음 단계로 옮기는 일은 결과의 책임이 사람에게 있으므로 사람이 확인하고 사람이 실행한다.
...
- 이번에 판단이 필요했던 지점 세 개를 여덟 항목 형식으로 로그에 적는다. 지원하지 않기로 한 이유, 근거가 약한 Aim을 어떻게 다룰지 정한 대목, 확정되지 않은 값을 어떻게 표시했는지가 그런 지점이다.
- 문헌 위키로 되돌릴 것과 과제 자리에 남길 것을 구분해 한 줄씩 적는다. 출판된 지식과 재사용 가능한 개념만 위키로 옮기고, 예산과 개인정보와 미공개 원고는 옮기지 않는다.

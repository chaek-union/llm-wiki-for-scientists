...
되돌린 자국도 남아 있다. 일정 변경을 코드로 막은 가드는 처음부터 있던 것이 아니라 같은 사고가 반복된 뒤에 만들어졌다. 요청의 단계를 식별하는 키를 두고, 완료되었거나 위임되었거나 외부 대기이거나 억제된 상태는 같은 키로 다시 열 수 없게 했다. 사람이 자동 생성된 일정을 지우면 지웠다는 표식이 남아 같은 키가 다시 만들어지지 않는다. 운영 로그에는 아침에 허용되어 있던 동작이 같은 날 저녁에 읽기 전용으로 바뀐 기록도 있다. 권한이 좁아지는 방향으로 움직인 시점은 모두 사고 뒤였고, 미리 좁혀 둔 적은 없다. 이 순서를 뒤집을 수 있다고 믿지 않는 편이 낫다는 것이 3년 동안 남은 판단이고, 그래서 자동화를 설계할 때 실제로 정하는 것은 무엇을 자동으로 할지가 아니라 무엇이 잘못되었을 때 그 사실이 사람에게 어떻게 도착하는지다.

파트 3이 다룬 여섯 가지 일은 서로 다른 업무이면서 같은 요구를 받는다. 학생의 성장 기록, 공동연구의 약속, 프로젝트마다 접근권이 다른 지식 공유, 과제의 생애주기, 예약해 둔 반복 작업의 결과, 기관에 내는 서류는 모두 어느 공간이 그것을 소유하는지 정해져야 하고 거기 적힌 값이 어느 정본에서 왔는지 되짚을 수 있어야 한다. 여섯 장이 소유를 먼저 정하고 값과 출처를 대응시키고 확인할 수 없는 곳에서 멈추는 같은 순서를 반복한 이유가 여기에 있다. 1장에서 시작한 문제는 AI와 나눈 좋은 대화가 사라지는 이유가 기억이 놓일 자리가 없다는 데 있다는 것이었다. 스물아홉 개 장을 지나며 그 자리는 논문 한 편을 넣는 폴더에서 학생과 공동연구와 프로젝트와 과제와 서류를 소유하는 여러 작업 공간으로 늘어났다. 늘어난 것은 자리의 개수이고 원리는 그대로다. 연구책임자가 구조화하는 것은 무엇이 어느 공간에 남는지 정하는 규칙과, 에이전트가 매번 같은 순서로 확인하게 만드는 인터페이스다. 에이전트는 흩어진 자료를 모으고 빠진 것을 찾아내고 초안을 만들고 확인할 수 없는 곳에서 멈춘다. 학생에게 무엇을 요구할지, 공동연구에서 무엇을 약속할지, 어느 공고에 지원하지 않을지, 어떤 문서에 서명해 기관에 낼지는 연구책임자가 직접 정하고 그 결과를 책임진다.

...
성공 기준은 그 계약 문서만 읽은 새 세션의 에이전트가 두 가지를 할 수 있는지다. 오늘 인계 자료가 없거나 허용된 나이를 넘겼을 때 내용을 지어내지 않고 미확인으로 보고하는 것이 하나이고, 어떤 행동을 승인 없이 하면 안 되는지를 그 문서만으로 판정하는 것이 다른 하나다. 두 가지가 되지 않으면 최신성과 권한이 문서에 충분히 적히지 않은 것이다. 여기에 한 달에 한 번 무엇을 검토할지도 정해 둔다. 예약 목록과 실제 활성 작업이 일치하는지, 그리고 지난달에 이 자동화 때문에 실제로 바뀐 판단이 있었는지가 검토 대상이다. 뒤쪽이 계속 비어 있다면 그 자동화는 정보를 만들고 있을 뿐 결정을 돕고 있지 않다는 뜻이고, 그때는 그 작업을 끄는 편이 낫다. 자동화가 만드는 것은 사람이 결정해야 할 것의 목록이다. 그 목록의 각 항목이 언제 만들어진 자료에서 나왔는지와 누가 최종 결정하는지를 함께 적어 두는 일이 예약 자동화를 운영한다는 말의 실제 내용이다.

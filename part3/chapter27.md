...
차무희 아침 브리핑은 Codex가 관리하는 예약 작업으로 등록되어 있다. 설정 파일에는 작업 종류가 `cron`으로 적혀 있고 실행 시각은 월요일부터 금요일까지 오전 6시 10분이다. 사용자 `crontab`에는 별도의 항목이 없다. 예약 시각이 되면 Codex가 차무희 작업을 열고 자동화 운영 규칙과 당일 인계 파일을 읽는다. 유효한 Gmail, 카카오톡과 논문 수집 인계를 모아 브리핑을 만든 뒤 날짜별 `outputs/` 폴더에 Markdown 원문을 저장한다. 입력의 최신성, 실행한 변경, 확인 결과와 남은 문제는 같은 날짜의 `logs/` 파일에 적는다. 브리핑 파일이 정상적으로 저장된 다음에 Slack 전송을 시작한다. 보고서 생성과 Slack 전송을 따로 판정하므로 Slack이 잠시 막혀도 완성된 브리핑 원문은 로컬에 남는다.

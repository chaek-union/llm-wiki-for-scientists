...
행정 서류 처리에서 반복되는 오해가 다섯 가지 있다. 첫째, 내용이 정확하면 서식은 부차적이라고 보는 것이다. 기관 양식에서는 서식 자체가 요구사항의 일부여서, 칸을 합치거나 줄을 늘리거나 표의 폭을 바꾸면 내용이 옳아도 반려되고 반려는 마감 뒤에 통보되는 경우가 많다. 둘째, 다루기 쉬운 형식으로 바꿔 편집한 뒤 되돌리면 된다고 생각하는 것이다. 형식 사이의 왕복 변환은 표의 폭, 줄바꿈 위치, 개체의 좌표를 조용히 바꾼다. 변환기가 오류를 내지 않았다는 것은 파일이 만들어졌다는 뜻이고, 배치가 보존되었다는 보장은 어디에도 없다. 셋째, 구조 검사를 통과하면 문서가 맞다고 보는 것이다. 구조 검사는 파일이 규격에 맞고 정상적으로 열리는지를 보는 층이고, 기관 담당자가 보는 것은 조판된 결과다. 두 층은 서로를 대신하지 못하므로 눈으로 확인하는 단계를 생략하면 검증은 절반만 끝난 것이다. 넷째, 개인 항목은 한 번 확인했으니 다음에도 그대로 쓰면 된다고 보는 것이다. 소속, 직급, 주소, 연락처는 본인이 인지하지 못하는 사이에 바뀌고 기관 명칭도 개편으로 달라지는데, 두 정본에서 다시 읽는 비용은 몇 분이고 틀린 값으로 제출한 뒤 정정하는 비용은 그보다 훨씬 크다. 다섯째, 에이전트가 채웠으니 사람의 검토는 형식적이라고 보는 것이다. 서명이 들어간 문서에 허위 진술이 있으면 그 책임은 서명한 사람에게 있고, 처리 과정이 자동이었다는 사실은 책임을 덜어 주지 않는다. 검토는 채운 값을 눈으로 다시 읽는 일이며, 이 단계를 생략할 수 있는 서류는 없다.

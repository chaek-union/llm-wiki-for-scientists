...
답은 전부 review 원문으로 되돌아가 확인한다. 특히 네 번째 질문의 답으로 나온 논문 목록은 검증 없이 받아들이면 안 된다. 에이전트가 중요하다고 말한 논문이 review 본문에서 실제로 중심 근거로 쓰였는지, 아니면 도입부에서 한 번 스쳐 간 인용인지는 원문을 열어야 알 수 있다. 두 경우를 구분하지 않으면 읽기 대기열의 앞자리가 배경 인용으로 채워지고, 정작 review의 논지를 지탱하는 논문은 뒤로 밀린다. 검색 도구는 후보를 좁혀 줄 뿐이며 그 역할과 한계는 5장에서 다뤘다. 여섯 질문의 답은 대화창에 두지 않고 곧바로 한 문서에 옮긴다. 대화창에 남긴 답은 다음 세션에서 사라지고, 사라지면 같은 질문을 다시 던지게 되며, 두 번째 답이 첫 번째와 다르면 어느 쪽이 맞는지 판단할 근거가 없다. 답마다 원문의 어느 절에 근거가 있는지를 함께 적어 두면 나중에 확인하는 비용이 크게 줄어든다. 그렇게 만들어진 문서가 이 장의 마지막 산출물인 field map의 초안이 되고, 초안 상태에서 이미 빈칸이 어디인지 드러난다. 빈칸은 아직 읽지 않은 원저가 채울 자리이므로 대기열을 짜는 근거가 된다.
...
이미 위키에 들어와 있는 review 한 편을 대상으로 한다. 아직 없으면 4장의 절차로 한 편을 먼저 들여놓고, 그 작업은 이 실습 시간에 포함하지 않는다. 여기서 만드는 것은 세 개의 문서다.

1. 앞 절의 여섯 질문을 순서대로 던지고 답을 받는다. 답마다 review 원문의 어느 절에 근거가 있는지 함께 요구한다. 근거 위치가 나오지 않은 답은 채택하지 않는다.
2. 네 번째 질문의 답으로 나온 논문 후보 가운데 세 편에서 다섯 편을 고른다. 고른 이유를 각각 한 줄로 적는다. 이유는 "review의 어떤 문장을 확인하려고"의 형태로 적는다.
3. 고른 목록을 읽기 대기열 문서로 저장한다. 순서와 이유가 함께 들어간다.
4. Field map 초안을 한 장으로 쓴다. 여섯 항목 중 지금 채울 수 없는 칸은 비워 두고 왜 비었는지 적는다.
5. 채우지 못한 칸에서 미해결 질문 세 개를 뽑아 별도 문서로 저장한다.

성공 기준은 새 세션을 시작한 에이전트에게 field map과 읽기 대기열만 주었을 때, 다음에 읽을 논문 한 편과 그 논문을 읽는 이유를 스스로 제시할 수 있는지다. 제시하지 못한다면 대기열의 이유 칸이 비어 있거나 field map이 나열에 머물러 있다는 뜻이다.

# 흔한 오해

온보딩 단계에서 반복되는 오해가 다섯 가지 있다. 첫째, 논문의 형식을 알면 논문을 읽을 수 있다고 보는 것이다. 절의 이름과 순서를 아는 일은 30분이면 끝나고, 그 지식은 어느 문장이 결정적인지 알려 주지 않는다. 형식 설명으로 끝나는 논문 읽기 수업이 학생을 오래 붙잡아 두는 이유가 여기에 있다. 둘째, 저널의 명성과 인용 수를 중요성으로 바꿔 읽는 것이다. 두 지표는 후보를 좁히는 데는 쓸모가 있지만 이 논문이 내 질문에 무엇을 해 주는지는 말해 주지 않는다. 인용이 적어도 자기 문제의 전제를 흔드는 논문이 있고, 인용이 아주 많아도 배경 인용으로만 쌓인 논문이 있다. 셋째, review의 해석을 분야의 최종 합의로 받아들이는 것이다. Review에는 저자의 관점과 범위 설정이 들어가 있고, 같은 시기에 나온 다른 review가 같은 쟁점을 다르게 정리하는 일도 흔하다. Review가 미해결이라고 적은 것을 미해결로, 해결되었다고 적은 것을 확인 대상으로 두는 편이 안전하다. 넷째, 에이전트가 추천한 논문 제목을 모으는 일을 온보딩이라고 부르는 것이다. 제목 목록은 몇 분이면 만들어지고 지도를 바꾸지 않는다. 원문을 열어 그 논문이 review의 어느 주장을 지탱하는지 확인하기 전까지 그 제목은 후보일 뿐이며, 후보와 근거를 구분하지 않으면 위키에 검증되지 않은 관계가 쌓인다. 다섯째, 과학적 리더십을 자신감이나 발표 태도의 문제로 보는 것이다. 목소리를 키우는 훈련으로는 근거가 어디까지 지탱하는지에 대한 판단이 생기지 않는다. 리더십은 자기 결과가 기존 설명을 강화하는지 좁히는지 반박하는지를 근거를 들어 말할 수 있는 상태를 뜻하고, 그 상태는 읽은 것을 관계로 기록하는 반복에서 나온다.

# 내 위키에 남기기

이 장의 결과는 세 개의 문서로 남긴다.

- Field map을 한 장짜리 문서로 저장하고, 첫 줄에 작성일과 어느 review를 기준으로 그렸는지 적는다. 기준을 적어야 나중에 지도가 낡았을 때 어디서부터 다시 그릴지 알 수 있다.
- 읽기 대기열을 별도 문서로 저장한다. 각 항목에 순서와 이유, 그리고 읽은 뒤에 field map의 어느 칸을 고쳤는지 기록할 빈칸을 둔다.
- 미해결 질문 목록을 저장하고, 각 질문이 어느 논문을 읽다가 나왔는지 표시한다. 출처가 없는 질문은 다시 봤을 때 왜 그것이 궁금했는지 복원되지 않는다.

세 문서는 review 페이지와 양방향으로 연결한다. 원저를 한 편 넣을 때마다 field map의 한 칸이라도 고쳤는지 확인하고, 고칠 것이 없었다면 그 사실을 대기열의 해당 항목에 적는다. 아무것도 바뀌지 않은 논문이 세 편 이어지면 대기열의 우선순위가 잘못되었다는 신호다. 파트 2의 나머지 장은 이 세 문서를 계속 쓴다. 9장에서 방법을 자기 설계로 바꿀 때 대기열이 출발점이 되고, 11장에서 주장을 조직할 때 미해결 질문 목록이 고찰의 재료가 된다. 앞부분을 복원해 한 장에 그려 두면 다음 논문이 그 대화의 다음 발언으로 읽힌다.

...
# 10장. 미팅과 공동연구를 설계하는 법

미팅은 서로 다른 배경과 정보를 가진 사람들이 같은 질문과 같은 근거 위에 서서 판단하고 다음 행동을 정하는 자리다. 결정을 남기는 힘은 미팅 전에 공유된 문서에서 나온다. 사전 자료 한 장은 30분의 배경 설명을 10분의 결정으로 바꾸고, 그 한 장을 만드는 책임은 자료를 들고 들어가는 사람에게 있다. 어떤 대학원생이 석 달치 분석 결과를 들고 미팅에 들어간다. 슬라이드는 스무 장이 넘고 각 장에는 실제로 수행한 작업이 시간순으로 들어 있다. 데이터를 어떻게 받았고, 어떤 필터를 걸었고, 중간에 파이프라인을 한 번 바꿨고, 그래서 다시 돌렸다는 이야기가 앞의 열 장을 채운다. 한 시간짜리 미팅에서 절반이 그 열 장에 쓰이고, 정작 판단이 필요한 뒤쪽 결과는 넘기듯 지나간다. 미팅은 "다음에 한 번 더 보자"로 끝나고, 참석자들은 서로 다른 것을 기억한 채 방을 나선다. 학생은 성실했고 결과도 있었다. 없었던 것은 무엇을 결정해 달라는 문장 하나와, 그 결정에 필요한 근거만 골라 놓은 종이 한 장이었다.

# 미팅은 정보 전달이 아니라 판단의 압축이다
...
<!-- AUTHOR: 학생 지도에서 반복해 관찰된 것 — 사전 정리를 요구했을 때 학생들이 가장 자주 막히는 항목이 무엇이었는지, 그리고 그 항목을 쓰게 하는 데 어떤 질문이 실제로 효과가 있었는지 한두 단락 -->
...
위키에서 근거를 모을 때는 검색어를 완전한 질문으로 던진다. 5장에서 다룬 방식이고, 미팅 준비에도 그대로 쓰인다. 관련된 overview와 concept 문서, 방법을 정리한 문서, 개별 논문의 source note를 훑어 이번 결정과 맞닿은 대목을 표시한다. 에이전트는 후보를 모으고 중복을 걸러 내고 형식을 맞추는 일을 빠르게 해내지만, 다섯 개의 후보 중 어느 둘이 이번 결정에 실제로 걸리는지는 판단해야 한다. 그 판단이 사전 자료를 만드는 사람의 몫이고, 위임하면 자료가 길어지면서 초점이 사라진다. 근거를 옮길 때는 원문으로 되돌아가는 경로를 함께 남긴다. 사전 자료에 적힌 한 문장짜리 판단이 어느 논문의 어느 결과에서 나왔는지 표시되어 있으면, 미팅 중에 이견이 생겼을 때 그 자리에서 확인할 수 있다. 경로가 없으면 "제가 어디선가 봤는데"로 시작하는 대화가 이어지고, 그 논의는 대개 결론 없이 끝난다. 공개된 문헌 지식과 프로젝트 내부의 미공개 결과도 서식으로 구분한다. 둘을 섞어 놓으면 나중에 이 문서의 어느 부분을 외부에 공유해도 되는지 다시 판단해야 하고, 급할 때 그 판단은 자주 틀린다.
...
<!-- AUTHOR: 공동연구에서 사전 자료의 유무가 첫 미팅의 결과를 갈랐던 관찰. 어떤 조건에서 사전 공유가 실제로 작동했고 어떤 조건에서는 형식만 남았는지에 대한 저자의 판단 -->
...
실제로 예정된 미팅 하나를 골라 준비부터 정리까지 한 번 수행한다. 아직 미팅이 없다면 다음 연구실 미팅을 대상으로 삼는다.

1. 미팅 전 여섯 문장을 쓴다. 결정 항목이 하나에서 셋 사이가 되도록 좁힌다.
2. 위키에서 이번 결정과 직접 연결된 근거만 골라 여덟 항목 골격에 맞춘 사전 자료 한 장을 만든다. 근거마다 원문으로 가는 경로를 남긴다.
3. 슬라이드를 네 장에서 여섯 장으로 만든다. 각 장의 제목은 그 장이 지지하는 결론으로 단다.
4. 미팅 최소 열두 시간 전에 사전 자료를 공유한다.
5. 동료 한 명에게 먼저 발표하고 이해가 막힌 지점을 기록해 고친다.
6. 미팅 중에는 근거, 해석, 결정 세 칸으로 노트를 적는다.
7. 당일 안에 후속 행동 목록을 만들고, 위키로 되돌릴 지식과 프로젝트 기록에만 남길 내용을 나눈다.

성공 기준은 미팅이 끝난 뒤 새 세션을 시작한 에이전트에게 후속 행동 목록만 주고 "이번에 무엇이 결정되었고 다음에 무엇을 해야 하는가"를 물었을 때, 다른 자료를 찾지 않고 답할 수 있는지다. 한 걸음 더 나아간 기준은 그 목록으로 다음 미팅 사전 자료의 첫 항목을 바로 채울 수 있는지다. 채울 수 없다면 결정의 범위나 책임자, 조건 중 하나가 비어 있는 것이다.

# 흔한 오해

첫째, 사전 자료를 보내면 미팅에서 할 말이 없어진다는 걱정이다. 실제로는 배경 설명이 사라진 자리에 상대의 질문과 반론이 들어오고, 그 논의가 미팅에서만 가능한 부분이다. 사전 자료는 발표를 앞부분부터 시작하지 않게 해 준다. 둘째, 자료를 많이 준비할수록 준비를 잘한 것이라는 생각이다. 슬라이드 스무 장은 성실함의 증거일 뿐이고, 결정에 필요 없는 근거가 자리를 차지하면 필요한 근거가 검토받을 시간이 줄어든다. 준비의 척도는 뺀 판단의 개수다. 셋째, 결정은 지도교수나 공동연구자가 하는 것이니 학생은 자료만 모으면 된다는 생각이다. 정리되지 않은 자료를 받은 상대는 정리부터 시작해야 하고, 그 정리는 데이터를 직접 다루지 않은 사람이 하는 것이므로 부정확하다. 스스로 확인할 수 있는 범위를 먼저 확인하고 남은 지점을 지목해 묻는 것까지가 사전 정리이고, 그 책임은 자료를 만든 사람에게 있다. 넷째, 녹음과 자동 요약이 미팅 기록 문제를 해결한다는 기대다. 전사는 발언을 남기지만 합의 범위와 책임자를 남기지 않고, 요약이 합의를 정확히 잘랐는지는 사람이 다시 확인해야 한다. 무엇을 기록할지 정하는 일과 기록하는 손을 줄이는 일은 다른 문제이며, 앞의 문제가 풀리지 않으면 뒤의 도구는 발언록의 양만 늘린다. 다섯째, 에이전트가 만든 사전 자료를 그대로 공유해도 된다는 판단이다. 초안의 문장은 매끄럽지만 판단의 강도가 실제보다 세거나 약하게 표현되어 있는 경우가 많고, 그 차이는 읽는 사람의 결정을 바꾼다. 발표자가 그 자리에서 근거를 대며 방어할 수 없는 문장은 공유 전에 빼는 편이 낫다.

# 내 위키에 남기기

이번 미팅에서 만든 네 가지를 파일로 남긴다. 미팅 전 여섯 문장, 사전 자료 한 장, 근거와 해석과 결정을 나눠 적은 세 칸 노트, 후속 행동 목록이다. 네 파일은 프로젝트 아젠다 폴더에 두고 미팅 날짜로 묶는다. 여기에 두 가지를 덧붙인다. 하나는 위키로 되돌릴 지식이다. 미팅을 준비하며 새로 들인 논문과 상대의 지적으로 바뀐 방법 이해를 해당 개념 문서와 방법 문서에 반영한다. 반영하지 않으면 다음 프로젝트에서 같은 지적을 다시 받는다. 다른 하나는 이번 준비에서 반복될 것 같은 판단 기준 세 개다. 어떤 근거를 사전 자료에 넣고 뺐는지, 결정 항목을 어떻게 좁혔는지, 무엇을 프로젝트 기록에만 두기로 했는지가 그런 기준이다. 같은 판단이 세 번쯤 반복되면 그 기준을 운영 규칙으로 옮긴다. 미팅마다 처음부터 판단하는 일이 줄어드는 만큼 준비 시간이 짧아진다. 미팅이 결정을 남기는 자리가 되는 것은 미팅장에서 정해지지 않는다. 무엇을 결정할지 미리 적고, 그 결정에 필요한 근거만 골라 하루 전에 건네고, 결정과 책임자와 다음 조건을 그날 안에 적어 두는 일에서 정해진다.

...
*Source note는 논문의 신원과 출처, 한 문장 요약, 방법·결과·한계를 위에서 아래로 배치한다. 같은 항목을 유지해야 여러 논문을 나란히 비교할 수 있다.*
...
*Ingest 뒤에는 요약만 읽지 않고 연구 대상, 방법, 수치, 한계, 관련 연구가 실제 원문에 근거했는지 항목별로 확인한다.*
...
*작업 로그에는 만든 파일, 분류 근거, 추출 도구, 연결한 개념, 적지 않기로 한 내용을 남긴다. 실행한 일과 제외한 판단이 함께 있어야 작업을 재현할 수 있다.*
...
먼저 세 파일이 같은 논문을 가리키는지 확인한다. PDF와 source note와 wiki page의 stem이 같은지, 제목과 저자와 연도가 서로 어긋나지 않는지 본다. 다음으로 필수 항목이 비어 있는지 확인한다. 항목 제목만 있고 내용이 없는 부분, 추출 오류가 그대로 들어온 부분, 판정이 빠진 부분을 찾는다. 링크와 카테고리가 실제로 존재하는지도 확인한다. 없는 문서를 가리키는 링크는 나중에 찾기 어렵다. 위키링크의 형태도 이때 눈으로 본다. 검사 스크립트가 접두사 붙은 형태를 통과시키므로 이 확인만은 사람이 대상 경로를 직접 읽는 수밖에 없다. 여기까지가 파일 자체에 대한 검증이고 다음은 이 작업을 기록으로 남기는 일이다.

작업 로그에는 무엇을 왜 추가했는지 남긴다. 파일 복구에는 동기화 서비스의 버전 기록을 쓰지만, 그 기록은 파일이 어떻게 바뀌었는지만 보여 주고 변경 이유는 알려 주지 않는다. 로그 파일은 하루에 하나가 아니고 하루와 에이전트와 기계의 조합마다 하나다. 이름은 날짜, 에이전트 이름, 기계 이름을 이어 붙여 만든다. 에이전트 이름은 `claude` 또는 `codex`로 적는다. 기계 이름이 파일명에 들어가는 이유는 두 대의 Mac이 Dropbox로 같은 폴더를 공유하기 때문이다. 기계 이름을 빼면 같은 날 두 대에서 돌린 작업이 한 파일에 동시에 쓰이고 동기화는 그 충돌을 충돌 사본으로 처리한다. 기계 이름은 기억에 의존하지 않고 `scutil --get ComputerName`을 실행해 확인한다. 에이전트는 자신이 어느 기계에서 실행되는지 안다고 착각하기 쉽다.

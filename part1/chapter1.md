...
성공 기준은 세 주제 중 최소 하나에서 사라진 것이 판단이나 맥락이었다는 점을 스스로 확인하는 것이다. 사실은 다시 찾을 수 있지만 판단과 맥락은 다시 만들어야 하고, 그것이 이 책 전체가 다루는 문제다. 쓴 선언문은 `readme` 또는 `목적`이라는 이름의 Markdown 문서로 저장해 앞으로 만들 위키의 첫 문서로 삼고, 세 주제와 각각에서 사라진 것을 표로 정리해 같은 문서 아래에 붙인다. 2장에서 폴더를 만들 때 이 표가 무엇을 어디에 둘지 정하는 근거가 된다.

...
Codex 안에서는 복잡한 판단과 대규모 교정에 Sol을 먼저 고른다. 일상적인 논문 ingest와 위키 페이지 작성에는 Terra로도 충분히 좋은 품질이 나온다. 같은 논문 28편을 Sol과 Terra Extra High로 각각 처리해 비교했을 때 두 모델 모두 논문 신원, 사실, 문서 구조에서 중대한 오류가 없었다. Terra는 근거 추적성, 방법의 구체성, 수치의 범위, 한계 처리를 더 자세히 기록했고 Sol은 더 간결했다. 논문 28편의 비교만으로 모든 작업에서 어느 모델이 나은지 단정할 수는 없다. 다만 Terra를 일상적인 위키 생성에 써도 된다고 판단할 근거는 얻었다.
...
이 책의 LLM-Wiki를 지속해서 운영하려면 월 20달러 이상의 유료 구독이 필요하다. 무료 등급으로는 논문을 여러 편 처리하고 폴더의 문서를 반복해서 고치는 작업량을 감당하기 어렵다. Codex를 쓸 사람은 ChatGPT 유료 구독을, Claude를 쓸 사람은 Claude 유료 구독을 준비한다.

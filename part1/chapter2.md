...
사람이 처음 정할 것은 작업 폴더 하나다. ChatGPT 데스크톱 앱에서 Codex의 새 세션을 열고 폴더 선택 화면의 `New Folder`를 눌러 `LLM-wiki`라는 폴더를 만든 뒤 그 폴더를 선택한다. 로컬 프로젝트를 이미 만들었다면 프로젝트 메뉴의 `Edit project → Add folder`에서 같은 폴더를 붙이고 기본 폴더로 지정한다. [Codex 공식 설명](https://learn.chatgpt.com/docs/projects#use-local-projects-for-folders-and-codebases)에 따르면 새 세션은 기본 폴더에서 시작하고 Codex는 그 폴더의 `AGENTS.md`와 설정을 자동으로 찾는다. 여기까지가 사람이 직접 하는 설정이다. 3장의 설치 요청을 실행하면 에이전트가 `AGENTS.md`, `CLAUDE.md`, `index.md`와 위키에 필요한 하위 폴더를 만든다. Codex는 `AGENTS.md`를 읽고 Claude Cowork와 Claude Code는 `CLAUDE.md`를 읽어 같은 작업 규칙을 따른다. 파일 준비와 문서 구조 설정은 에이전트에게 맡긴다. 지침 파일과 목차 문서도 에이전트가 만든다.

작업이 달라질 때마다 새 세션을 연다. 논문을 넣는 일, 특정 질문을 조사하는 일, 여러 문서를 고치는 일을 각각 새 세션으로 시작해도 같은 `LLM-wiki` 폴더를 선택하면 기존 문서를 함께 읽을 수 있다. 로컬 프로젝트에서 `LLM-wiki`를 기본 폴더로 지정해 두면 새 세션도 그 폴더에서 시작한다. 세션마다 대화 기록은 따로 남지만 파일은 같은 폴더를 사용한다. 새 세션의 에이전트는 폴더 안의 지침과 문서를 읽어 이전 작업의 결과를 이어 간다. 같은 작업을 계속할 때는 이전 세션을 다시 열어도 되고 작업 주제가 바뀌었으면 새 세션을 열면 된다. 오래 남아야 하는 판단과 진행 상황은 대화 기록에만 두지 않고 에이전트가 폴더의 Markdown 문서와 작업 로그에 적게 한다. 세션이 길어져 앞부분의 설명을 다시 찾기 어려워졌을 때도 새 세션에서 같은 폴더를 열면 된다. 모든 세션이 같은 작업 폴더를 읽도록 지정하면 작업 기록이 세션이 바뀐 뒤에도 이어진다.

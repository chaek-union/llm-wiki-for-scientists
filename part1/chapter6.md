...
part: 1
budget: 17000
evidence: [E-030, E-031, E-032, E-033, E-034, E-035, E-036, E-037, E-038, E-039, E-040]
---

# 6장. 논문의 구조를 해부하기

이 장은 한 편의 논문이 무엇 위에 서 있는지를 읽는 법을 다룬다. 참고문헌 목록에는 논문의 동기와 방법과 데이터와 비교가 조립되어 있는데, 결과 요약은 그 조립을 보여 주지 않는다. 이 장을 마치면 논문 한 편에서 요약으로는 나오지 않는 네 가지를 꺼낼 수 있게 된다. 지적 계보, 신뢰를 만드는 방식, 학습 자료의 출처 구조, 그리고 자기 위키에 비어 있는 자리다. 절차는 여섯 묶음이고, 마지막 묶음은 다음에 읽을 논문의 목록을 만든다. 대학원 논문 발표 수업에서 반복해서 보게 되는 장면이 있다. 발표자가 분야를 대표하는 모델 논문 한 편을 맡아 30분 동안 잘 만든 슬라이드를 넘긴다. 문제 설정, 구조 그림, 성능 표, 저자가 밝힌 한계까지 빠진 것이 없다. 그런데 청중에서 "이 논문은 왜 하필 그 모델들과 비교했는가", "학습에 쓴 데이터는 누가 모은 것인가", "저자들이 넘어서려는 기존 관점은 어느 논문에 있는가" 같은 질문이 나오면 발표가 멈춘다. 발표자가 논문을 읽는 동안 그 질문을 하지 않았기 때문이고, 세 질문의 답은 모두 참고문헌 목록과 본문 사이의 대응 관계에 들어 있다.
...
<!-- AUTHOR: Geneformer를 읽다가 이 절차를 이름 붙은 방법으로 만들게 된 계기. 요약을 아무리 잘 써도 답이 나오지 않던 질문이 무엇이었는지, 참고문헌을 처음 끝까지 대응시켜 보았을 때 무엇이 보였는지 한두 단락 -->

# 인용이 맡는 여덟 가지 기능

구조 분석의 첫 도구는 인용이 그 논문 안에서 하는 일에 이름을 붙이는 것이다. 기능은 지금 분석 중인 논문 안에서 그 인용이 맡은 역할이다. 같은 논문이 어떤 논문에서는 비교 대상 baseline으로, 다른 논문에서는 label을 공급한 자료로, 또 다른 논문에서는 배경 생물학으로 인용된다. 4장에서 다룬 category와 혼동하지 않아야 한다. Category는 위키 전체에서 그 논문을 어디에 두고 어떤 질문으로 다시 찾을지에 대한 결정이고, 기능 분류는 논문 한 편 안에서의 위치다. 하나는 위키의 좌표이고 다른 하나는 논증의 좌표다. 축이 고정되어 있어야 논문 사이의 비교가 성립한다. 논문마다 다른 눈으로 읽으면 인상이 여러 개 생기고 그 인상들은 서로 비교되지 않는다. 축을 늘리는 것은 기존 축 어디에도 들어가지 않는 역할이 실제로 발견되었을 때이고, 다섯 편을 분석하는 동안 그런 일이 한 번 있었다. 이름의 목록은 여덟 개이고, 각 이름은 그 자리에서 던져야 할 질문과 짝을 이룬다.
...
구하지 못한 인용은 구하지 못했다고 적는다. 결과물에 접근 불가 목록이 그대로 남고, 그 목록 자체가 정보다. 가장 나쁜 결과는 읽지 못한 논문에 대해 제목과 초록에서 추론한 그럴듯한 두 문장을 써 넣는 것이다. 그 두 문장은 검증할 수 없고, 다음에 이 페이지를 읽는 사람은 원문을 확인한 서술과 구분하지 못한다. 확인한 것과 확인하지 못한 것의 경계를 문서에 남기는 일이 이 방법이 지키려는 최소한이다. 인용 전체의 원문을 읽을 수는 없으므로 무엇이 논지를 떠받치는 인용인지 판단해야 한다. 판단 기준은 논증이 그 인용에 실제로 기대고 있는지다. 정량 비교의 대상이 된 baseline, label을 공급한 자료, 동기에서 넘어서겠다고 지목한 대비 대상, 결과 해석의 틀이 여기에 해당한다. Methods의 자료 출처 꼬리는 대개 여기에서 빠진다. 다만 그 꼬리를 읽지 않는 것과 세지 않는 것은 다른 문제이고, 크기와 구성은 반드시 센다.

<!-- AUTHOR: 학생들이 이 단계를 건너뛰는 지점에 대한 지도 경험. 인용된 논문의 원문을 읽으라고 했을 때 실제로 돌아오는 결과물이 어떤 모습이었는지, 어떤 확인 질문을 던졌을 때 읽었는지 아닌지가 드러났는지 한 단락 -->
...
신뢰 전략과 나란히 놓이는 두 번째 비교축은 자료 출처를 안에 적는가 밖으로 내보내는가다. Geneformer는 Methods에 110건 남짓의 출처 인용을 달아 corpus 조립을 본문 안에 기록했고, AlphaGenome은 같은 종류의 기록을 Supplementary 목록으로 통째로 넘겼다. 참고문헌의 길이만 비교하면 앞의 논문이 훨씬 넓게 읽은 것처럼 보이지만, 같은 축으로 재면 차이는 출처를 어디에 적었는가에 있다. 개별 데이터셋 대신 집계 repository를 인용해 꼬리를 압축한 scFoundation은 둘 사이에 있다. 세 논문을 나란히 놓으면 자료 출처를 참고문헌에 남기는 관행이 어느 방향으로 움직이고 있는지가 보인다. 세 번째 비교축은 계보의 폭이다. NTv3에서는 자기 인용이 계보를 지배했는데, 한 연구집단이 이전에 내놓은 여러 유전체 모델을 하나의 backbone으로 통합하는 작업이었기 때문이다. Method lineage 축의 상당 부분이 같은 집단의 이전 논문으로 채워지는 모습은 여러 전통을 봉합하는 논문과 형태가 확연히 다르다. 자기 인용이 많다는 사실은 그 논문이 무엇을 하는 작업인지를 말해 줄 뿐 좋고 나쁨을 판정하지 않는다. 통합형 논문은 외부 전통과의 접점이 적으므로, 그 모델을 다른 맥락에 옮길 때 무엇이 함께 따라와야 하는지를 별도로 확인해야 한다.

세 축의 비교가 가능했던 것은 여덟 개의 축이 다섯 편 내내 고정되어 있었기 때문이다. 논문마다 그때그때 인상적인 것을 적었다면 다섯 개의 인상이 생기고 서로 비교되지 않았을 것이다. 5장에서 여러 논문을 나란히 놓고 종합할 수 있었던 조건과 같다. 비교를 만드는 것은 축의 고정이고, 통찰은 축이 고정된 뒤에 축 사이의 관계에서 나온다. 여섯 번째 논문을 같은 축으로 재면 세 갈래 중 하나에 들어가거나 네 번째 갈래를 만들 텐데, 어느 쪽이든 앞의 다섯 편에 대한 이해가 함께 갱신된다.

<!-- AUTHOR: 인용 수를 영향력이나 타당성의 지표로 쓰는 관행에 대한 저자의 입장과 그 수위. 학계에서 이 지표가 실제로 어떻게 쓰이는지, 구조 분석이 그 대체가 될 수 있다고 주장할 것인지 아니면 다른 종류의 정보라고만 할 것인지 한 단락 -->

# 해보기

짧은 논문 한 편을 골라 Introduction과 첫 Results 하위 절만 대상으로 구조 분석을 수행한다. 논문 전체를 대상으로 삼으면 분량에 먼저 지치므로 범위를 좁게 잡는다. 이미 읽어 내용을 아는 논문을 고르는 편이 낫다.

1. 두 section에 등장하는 인용을 모두 뽑아 번호, 서지, 등장 위치, 그 자리에서 하는 일 한 줄로 표를 만든다.
2. 표에서 인용 10건을 골라 여덟 축 가운데 하나씩 배정한다. 배정이 애매한 것은 애매하다고 표시하고 넘어간다.
3. 10건 가운데 논지를 떠받치는 인용 2건을 고르고, 그 두 논문의 PDF 본문을 실제로 읽는다. 방법과 수치를 확인하고, 인용하는 쪽의 서술과 다른 점이 있으면 적는다.
4. 이 논문의 main-text 인용 수와 Methods 층 인용 수를 각각 센다. 두 수를 나란히 적고 비율이 무엇을 뜻하는지 한 문장으로 쓴다.
5. 인용 관계를 mermaid flowchart 한 장으로 그린다. 노드는 열 개를 넘기지 않는다.
6. 10건 가운데 지금 위키에 없는 것을 목록으로 만들고, 각 항목에 원문 확보 가능 여부를 적는다.

성공 기준은 6번 목록의 첫 항목을 그대로 다음 ingest 대상으로 넘길 수 있는지, 그리고 새 세션을 시작한 에이전트에게 "이 논문은 무엇으로 신뢰를 얻는가"를 물었을 때 3번에서 확인한 내용을 근거로 답할 수 있는지다. 3번을 건너뛰고 나머지를 다 채운 결과물은 이 기준을 통과하지 못한다.

# 흔한 오해

구조 분석을 시작할 때 반복해서 나오는 오해가 다섯 가지 있다. 첫째, 인용 수가 많은 논문이 중요한 논문이라고 보는 것이다. 인용 수는 그 논문이 얼마나 자주 호출되었는지를 셀 뿐이고, 어떤 자리에서 무슨 일을 하며 호출되었는지는 말해 주지 않는다. 방법을 물려준 논문과 도구 이름을 대기 위해 달린 인용이 같은 1로 세어진다. 구조 분석은 그 1들을 서로 다른 것으로 되돌리는 작업이다. 둘째, 인용하는 쪽의 문장을 풀어 쓰면 인용된 논문을 읽은 것이라고 여기는 것이다. 그 문장은 인용하는 쪽이 자기 논지에 맞게 각도를 잡은 요약이므로, 원문의 조건과 범위가 이미 한 번 걸러져 있다. 걸러진 문장을 다시 옮기면 오차가 두 번 쌓이고, 쌓인 결과가 위키에 사실처럼 남는다.

셋째, 기능 분류표를 다 채우면 구조 분석이 끝났다고 보는 것이다. 분류표는 인용의 무게중심을 잰다. 같은 인용이 section을 따라 이동하며 만드는 흐름은 지도와 도식에서 보인다. 넷째, Methods의 자료 출처 인용은 지엽적이므로 세지 않아도 된다고 보는 것이다. 그 꼬리의 크기와 구성이 논문의 성격을 결정하는 경우가 있고, 꼬리가 갑자기 짧아진 논문에서는 출처가 다른 곳으로 옮겨 갔을 뿐이다. 세지 않으면 두 상황을 구분할 수 없다. 다섯째, 원문을 구하지 못한 인용을 초록으로 메워도 된다고 보는 것이다. 초록은 저자가 고른 결론이고 방법과 조건이 빠져 있어, 두 논문의 관계를 판정하는 데 필요한 정보가 대개 거기에 없다. 구하지 못했다고 적힌 항목은 나중에 채울 수 있지만, 초록으로 메운 항목은 채워졌다고 표시되어 다시 열리지 않는다.

# 내 위키에 남기기

이 장의 결과는 문서 두 개로 남는다. 구조 분석 결과는 overview 층에 페이지 하나로 저장하고, 분석 대상 논문의 페이지와 양방향으로 연결한다. 페이지에는 지도, 기능 분류, 관계 도식, 해석, 그리고 원문을 구하지 못한 인용의 목록이 함께 들어간다. 마지막 항목을 빼면 나중에 이 페이지의 깊이가 어디까지 검증된 것인지 알 수 없게 된다. Coverage 목록은 별도의 대기열 문서로 남긴다. 항목마다 인용 정보, 원문 확보 가능 여부, 확보를 시도한 경로를 적는다. 다음 ingest는 이 대기열의 위에서부터 가져가고, 하나가 끝날 때마다 구조 분석 페이지의 해당 인용에 링크를 채운다. 대기열이 비면 그 논문의 계보가 위키 안에서 이어진 것이고, 비지 않는 항목이 남으면 그 부분이 이 위키가 아직 답할 수 없는 영역이다.

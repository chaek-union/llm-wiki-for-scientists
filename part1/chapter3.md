...
위 프롬프트를 입력하면 설치가 진행되는 동안 다음과 같은 질문이 나올 수 있다.

![설치 과정에서 연구 분야를 묻는 화면](../images/install-structure-check.png)

*설치 과정에서 에이전트가 위키의 연구 분야와 카테고리 기준을 묻는 화면이다. 자신의 연구에 가까운 항목을 고르거나 직접 입력하면 된다.*
...
설치가 끝났다는 것과 위키가 작동한다는 것은 다르다. 확인할 항목은 여섯 개이고 전부 30분 안에 끝난다.
...
지침 파일은 새 세션을 열어 실제로 적용되는지 확인한다. 나머지는 폴더와 지침 파일을 열어 위 목록대로 살펴보면 된다. 빈 폴더는 그대로 두고 원문이 없는 예제 페이지로 채우지 않는다. 조건을 충족하지 못한 항목이 있으면 논문을 넣기 전에 먼저 고친다.

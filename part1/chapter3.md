...
층을 나눠 놓아도 답을 만드는 규칙이 없으면 구조는 금방 무너진다. 위키에 없는 내용을 에이전트가 다른 데서 채우면 답은 그럴듯해지는데 위키는 그대로 비어 있고 비어 있다는 사실마저 답에 가려진다. [설치 Gist의 지침 파일](https://gist.github.com/joonan30/cbce305684d079dbe9a3fbaefe4e3959)에 적힌 네 가지 핵심 원칙은 답의 근거를 연구자가 실제로 소유한 논문 집합에 붙들어 맨다. 지침 파일에 넣어 세션마다 읽히게 하고 어긴 것이 보이면 그때 지적한다.
...
없다는 답을 받아들일 수 있는지가 이 방식이 작동하는지를 가른다. 논문이 없다는 답은 무엇을 구해야 하는지 알려 주므로 다음 동작으로 이어진다. 근거가 비어 있는 답을 다른 자료로 채우면 부족하다는 사실이 가려지고 나중에 원문을 열어 보기 전까지 알아채기 어렵다. 규칙은 넷으로 시작했지만 설치 Gist에 지금 적혀 있는 것은 아홉이다. 늘어난 다섯은 종합 문서와 연결되기 전에는 ingest를 완료로 보지 않기, 내용 없는 placeholder 문서 만들지 않기, 전수 작업에서 분모와 분자를 함께 밝히기, 비공개 자료를 `wiki/`에서 제외하기, 원문은 PDF만 쓰고 출판사 HTML이나 브라우저 캡처는 쓰지 않기다. 다섯 모두 무언가 잘못된 뒤에 생겼고 어느 규칙이 어느 사고에서 나왔는지는 로그를 거슬러 올라가면 대개 찾을 수 있다. 그래서 규칙 목록의 길이는 그 위키가 겪은 실패의 수를 나타낸다. 자기 위키의 규칙이 넷에서 늘지 않았다면 아직 실패를 만나지 않았거나 만난 실패를 규칙으로 옮기지 않은 것이다.

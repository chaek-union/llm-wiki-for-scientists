...
# 5장. 논문 여러 편을 한 번에 넣는 법

읽어야 할 논문은 한 편씩 오지 않는다. 학회에서 돌아오면 스무 편이 생기고, 리뷰 논문 하나의 참고문헌을 따라가면 마흔 편이 생기고, 새 주제를 시작하면 지난 3년치를 한꺼번에 훑게 된다. 4장의 절차를 스무 번 반복하는 것으로 이 상황이 정리되지는 않는다. 한 편을 넣을 때는 문제가 되지 않던 것들이 스무 편에서 한꺼번에 문제가 되기 때문이다. 같은 논문이 이미 들어 있는지 확인하는 일, 넣지 않기로 한 저널의 논문을 거르는 일, 추출이 조용히 실패한 파일을 찾아내는 일, 스무 편 전부가 종합 층과 이어졌는지 확인하는 일이 여기 해당한다. 이 장은 여러 편을 한 번에 넣을 때 추가로 필요한 절차를 다룬다. 품질 기준 자체는 4장과 같고, 달라지는 것은 그 기준이 지켜졌는지 확인하는 방법이다.
...
색인과 목록은 문서에서 다시 만들 수 있는 파생 상태이므로 정본과 다르게 다룬다. 파생 상태가 낡았을 때 고칠 것은 파생 상태이고, 정본이 틀렸을 때 고칠 것은 정본이다. 둘을 혼동하면 검색 결과가 이상할 때마다 문서를 고치게 되고, 문서는 멀쩡한데 색인만 낡은 경우가 대부분이므로 고칠 필요가 없던 문장이 고쳐진다. 배치 뒤에 검색이 이상하면 먼저 색인의 생성 시각을 본다. 이 구분과 재구축 시점은 6장에서 검색을 다루면서 다시 나온다.

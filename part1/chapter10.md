...
논문 한 편에서 실제로 다시 쓰이는 것은 본문 문장보다 표인 경우가 많다. 유전체 연구에서 메타분석을 하려면 여러 논문의 유전자 목록이 필요하고, 그 목록은 그림 안에 요약된 몇 개를 빼면 전부 보충 자료의 표에 있다. 코호트의 인원 구성, 세포 유형별 표지 유전자, 차등 발현 결과, 실험 조건별 처리 시간도 마찬가지다. 앞의 장들이 다룬 흐름은 원문 PDF에서 정리 문서로, 정리 문서에서 위키 페이지로 가는 한 방향이었다. 보충 자료는 그 흐름에 얹히지 않는다. 읽어서 판단으로 바꾸는 자료가 아니라 나중에 계산의 입력으로 다시 열리는 자료이기 때문이다. 이 장은 그 성격이 다른 층을 어디에 두고 어떻게 다시 찾을 수 있게 만드는지를 다룬다.

# 지식 층과 자원 층의 구분

보충 자료는 별도의 폴더가 소유한다. 원문 PDF를 두는 자리에 함께 두지 않는 이유는 두 가지다. 첫째는 세기 위해서다. 논문 한 편에 보충 파일이 스무 개 붙는 경우가 드물지 않고, 그것들이 같은 폴더에 있으면 논문이 몇 편인지 셀 수 없다. 4장에서 본 폴더 분리가 이 필요에서 갈라져 나왔다. 둘째는 성격이 다르기 때문이다. 원문에서 정리 문서로 이어지는 층은 그 논문이 무엇을 주장하는지를 나른다. 보충 자료 층은 다시 쓰이는 데이터를 나른다. 두 층에서 하는 일이 다르므로 찾는 방법도 달라야 한다. 주장을 찾을 때는 개념과 질문으로 찾고, 데이터를 찾을 때는 어느 논문의 어느 표인지로 찾는다.
...
보충 자료를 모을 때 세션에 주는 요청은 다음 정도면 된다.
